...
export class ImageRelayService {
  ...
  async upload(id: string, worker: string, contentType: string | undefined, req: NodeJS.ReadableStream) {
    ...
    const job = await this.job(id); if (job.status === 'completed') return this.asset(id);
  ...
  async complete(id:string, worker:string, body:any) { this.auth(worker); const job=await this.job(id); if(job.status==='completed') return this.asset(id); if (!body?.sha256 || Number(body.bytes)<=0) throw new BadRequestException('Metadata de imagen incompleta'); const key=`${id}/original`; const path=join(this.root,key); try { await fs.stat(path); } catch { throw new BadRequestException('Upload no encontrado'); } const result=await this.db.query(`INSERT INTO image_assets (inventory_id,image_job_id,source_url,storage_key,content_type,bytes,sha256) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (image_job_id) DO UPDATE SET updated_at=NOW() RETURNING *`,[job.inventory_id,id,job.image_url,key,body.contentType,body.bytes,body.sha256]); await this.db.query(`UPDATE image_jobs SET status='completed', completed_at=NOW(), updated_at=NOW(), claimed_by=$2 WHERE id=$1`,[id,worker]); return result.rows[0]; }
